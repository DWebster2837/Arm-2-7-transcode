...
//TODO: make system for handling multiple sections, rather than just using first program section.
char* headerdecode(char* buffer) {
	...
	if (!(buffer[0] == 0x7F &&
		...
		buffer[2] == 0x4c &&
		buffer[3] == 0x46)) {
		printf("File needs to be ELF format!");
		exit(0);
	}
	if (buffer[4] != 0x01) {
		printf("File is not 32-bit");
		exit(0);
	}
	if (buffer[5] != 0x01) {
	...
	if (buffer[12] != 0x28) {
		printf("File instruction set architecture is not ARMv7 or older");
		exit(0);
	}

	uint32_t flags = buffer[0x24];

	//program header table offset, entry size and entry count. Probably unused.
	uint32_t phoff = buffer[0x1C];
	uint16_t phentsize = buffer[0x2A];
	uint16_t phnum = buffer[0x2C];

	//section header table offset, entry size, entry count and section name entry index
	uint32_t shoff = buffer[0x20];
	uint16_t shentsize = buffer[0x2E];
	uint16_t shnum = buffer[0x30];
	uint16_t shstrndx = buffer[0x32];
